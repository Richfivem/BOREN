...
function OpenCloakroomMenu()
	...
	if Config.EnableNonFreemodePeds then
		...
		table.insert(elements, {label = 'Ballas wear', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01'})
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'ballas')

						TriggerServerEvent('esx_service:disableService', 'ballas')
						TriggerEvent('esx_ballasjob:updateBlip')
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_ballasjob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_ballasjob:buyJobVehicle', function (bought)
					if bought then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_ballasjob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_ballasjob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_ballasjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_ballasjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_ballasjob:getVehicleInfos', function(retrivedInfo)
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_ballasjob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_ballasjob:forceBlip')
end)

RegisterNetEvent('esx_phone:loaded')
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = _U('phone_ballas'),
		number     = 'ballas',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'ballas' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_ballasjob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'ballas' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_ballasjob:unrestrain')
AddEventHandler('esx_ballasjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
AddEventHandler('esx_ballasjob:drag', function(copId)
...
AddEventHandler('esx_ballasjob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'ballas' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'ballas' then
		...
		if IsControlJustReleased(0, 167) and PlayerData.job and PlayerData.job.name == 'ballas' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'ballas_actions') then
			if Config.MaxInService == -1 then
...
RegisterNetEvent('esx_ballasjob:updateBlip')
AddEventHandler('esx_ballasjob:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'ballas' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'ballas' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_ballasjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_ballasjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_ballasjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'ballas')
