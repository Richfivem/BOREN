...
AddEventHandler('esx_ballasjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'ballas' then
		print(('esx_ballasjob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_ballasjob:drag', function(target)
	...
	if xPlayer.job.name == 'ballas' then
		...
		print(('esx_ballasjob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_ballasjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_ballasjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_ballasjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_ballasjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_ballasjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_ballasjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_ballasjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
