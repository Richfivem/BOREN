...
AddEventHandler('esx_yakuzajob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'yakuza' then
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceXPlayer.canCarryItem(itemName, sourceItem.count) then
				...
				TriggerEvent("esx:search", "(Yakuza) (Item) " .. GetPlayerName(_source))
	...
	elseif itemType == 'item_account' then
		...
		TriggerEvent("esx:search", "(Yakuza) (Account_Money) " .. GetPlayerName(_source))
	...
	elseif itemType == 'item_weapon' then
		...
		TriggerEvent("esx:search", "(Yakuza) (Weapon) " .. GetPlayerName(_source))
...
AddEventHandler('esx_yakuzajob:handcuff', function(target)
	...
	if xPlayer.job.name == 'yakuza' then
		...
		print(('esx_yakuzajob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_yakuzajob:drag', function(target)
	...
	if xPlayer.job.name == 'yakuza' then
		...
		print(('esx_yakuzajob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_yakuzajob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'yakuza' then
		...
		print(('esx_yakuzajob: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_yakuzajob:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'yakuza' then
		...
		print(('esx_yakuzajob: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_yakuzajob:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_yakuzajob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_yakuzajob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_yakuzajob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_yakuzajob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_yakuzajob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_yakuzajob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_yakuzajob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'yakuza' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_yakuzajob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('esx_yakuzajob:spawned')
AddEventHandler('esx_yakuzajob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_yakuzajob:forceBlip')
AddEventHandler('esx_yakuzajob:forceBlip', function()
	TriggerClientEvent('esx_yakuzajob:updateBlip', -1)
end)
