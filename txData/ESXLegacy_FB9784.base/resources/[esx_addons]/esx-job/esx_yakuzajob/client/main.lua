...
function OpenCloakroomMenu()
	...
	if Config.EnableNonFreemodePeds then
		...
		table.insert(elements, {label = 'Yakuza wear', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01'})
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'yakuza')

						TriggerServerEvent('esx_service:disableService', 'yakuza')
						TriggerEvent('esx_yakuzajob:updateBlip')
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_yakuzajob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_yakuzajob:buyJobVehicle', function (bought)
					if bought then
...
function OpenYakuzaActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'yakuza_actions', {
		title    = 'Yakuza',
		align    = 'top-left',
		elements = {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						...
						--TriggerServerEvent('esx:search', "Yakuza")
						TriggerServerEvent('esx_yakuzajob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_yakuzajob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_yakuzajob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_yakuzajob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_yakuzajob:getVehicleInfos', function(retrivedInfo)
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_yakuzajob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_yakuzajob:forceBlip')
end)

RegisterNetEvent('esx_phone:loaded')
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = _U('phone_yakuza'),
		number     = 'yakuza',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'yakuza' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_yakuzajob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'yakuza' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_yakuzajob:unrestrain')
AddEventHandler('esx_yakuzajob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
AddEventHandler('esx_yakuzajob:drag', function(copId)
...
AddEventHandler('esx_yakuzajob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'yakuza' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'yakuza' then
		...
		if IsControlJustReleased(0, 167) and PlayerData.job and PlayerData.job.name == 'yakuza' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'yakuza_actions') then
			if Config.MaxInService == -1 then
...
RegisterNetEvent('esx_yakuzajob:updateBlip')
AddEventHandler('esx_yakuzajob:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'yakuza' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'yakuza' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_yakuzajob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_yakuzajob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_yakuzajob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'yakuza')
