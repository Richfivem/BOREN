...
AddEventHandler('esx_bloodsjob:handcuff', function(target)
	...
	if xPlayer.job.name == 'bloods' then
		...
		print(('esx_bloodsjob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_bloodsjob:drag', function(target)
	...
	if xPlayer.job.name == 'bloods' then
		...
		print(('esx_bloodsjob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_bloodsjob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'bloods' then
		...
		print(('esx_bloodsjob: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_bloodsjob:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'bloods' then
		...
		print(('esx_bloodsjob: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_bloodsjob:getFineList', function(source, cb, category)
...
ESX.RegisterServerCallback('esx_bloodsjob:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_bloodsjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_bloodsjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_bloodsjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_bloodsjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_bloodsjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_bloodsjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_bloodsjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'bloods' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_bloodsjob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('esx_bloodsjob:spawned')
AddEventHandler('esx_bloodsjob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_bloodsjob:forceBlip')
AddEventHandler('esx_bloodsjob:forceBlip', function()
	TriggerClientEvent('esx_bloodsjob:updateBlip', -1)
end)
