...
AddEventHandler('esx_cripsjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'crips' then
		print(('esx_cripsjob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
				...
				TriggerEvent("esx:search", "(Crips) (Item) " .. GetPlayerName(_source))
	...
	elseif itemType == 'item_account' then
		...
		TriggerEvent("esx:search", "(Crips) (Account_Money) " .. GetPlayerName(_source))
	...
	elseif itemType == 'item_weapon' then
		...
		TriggerEvent("esx:search", "(Crips) (Weapon) " .. GetPlayerName(_source))
...
AddEventHandler('esx_cripsjob:handcuff', function(target)
	...
	if xPlayer.job.name == 'crips' then
		...
		print(('esx_cripsjob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_cripsjob:drag', function(target)
	...
	if xPlayer.job.name == 'crips' then
		TriggerClientEvent('esx_cripsjob:drag', target, source)
	else
		print(('esx_cripsjob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_cripsjob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'crips' then
		...
		print(('esx_cripsjob: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_cripsjob:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'crips' then
		...
		print(('esx_cripsjob: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_cripsjob:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_cripsjob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_cripsjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_cripsjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_cripsjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_cripsjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_cripsjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_cripsjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_cripsjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'crips' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_cripsjob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('esx_cripsjob:spawned')
AddEventHandler('esx_cripsjob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_cripsjob:forceBlip')
AddEventHandler('esx_cripsjob:forceBlip', function()
	TriggerClientEvent('esx_cripsjob:updateBlip', -1)
end)
...
RegisterServerEvent('esx_cripsjob:message')
AddEventHandler('esx_cripsjob:message', function(target, msg)
